...
global.GenType = {
        Uuid(mode='db', defaultValue) {
            ...
            Uuid.gen = (fieldName) => {
                ...
                Uuid.assignUpController = Uuid.assignInController = `if(utils.has(req.body.${fieldName})) req.body.${fieldName} = db.uuid(${Uuid.sign}req.body.${fieldName});`;                    
                if (defaultValue === undefined) {
                    Uuid.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, db.Uuid);`;
                    }                    
                    Uuid.validateUpService = (item) => {
                        return `checker.option('${fieldName}', ${item}.${fieldName}, db.Uuid);`;
                    }                    
                    ...
                    Uuid.validateInService = Uuid.validateUpService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, db.Uuid, db.uuid(${defaultValue !== 'uuid' ? JSON.ostringify(defaultValue) : ''}));`;
                    }                    
        ...
        Key(type = GenType.Uuid, defaultValue) {   
            ...
            Key.gen = (fieldName) => {
                ...
                Key.validateInService = (item) => {
                    return `${item}.${fieldName} = db.uuid();`;
                }
                Key.validateDeService = Key.validateGeService = (item) => {
                    return `${item} = checker.must('${fieldName}', ${item}, db.Uuid);`;
                }
            ...
            String.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {
                    ...
                    String.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, String);`;
                    }
                    String.validateUpService = (item) => {
                        return `checker.option('${fieldName}', ${item}.${fieldName}, String);`;
                    }
                } else if (defaultValue === null) {
                    ...
                    String.validateUpService = String.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, String, ${JSON.ostringify(defaultValue)});`;
                    }
                }
            ...
            Number.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {
                    ...
                    Number.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, Number);`;
                    }
                    Number.validateUpService = (item) => {
                        return `checker.option('${fieldName}', ${item}.${fieldName}, Number);`;
                    }
                } else if (defaultValue === null) {
                    ...
                    Number.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, Number, ${JSON.ostringify(defaultValue)});`;
                    }
                    Number.validateUpService = (item) => {
                        return `checker.option('${fieldName}', ${item}.${fieldName}, Number);`;
                    }
            ...
            Boolean.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {
                    ...
                    Boolean.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, Boolean);`;
                    }
                    Boolean.validateUpService = (item) => {
                        return `checker.option('${fieldName}', ${item}.${fieldName}, Boolean);`;
                    }
                } else if (defaultValue === null) {
                    ...
                    Boolean.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, Boolean, ${JSON.ostringify(defaultValue)});`;
                    }
                    Boolean.validateUpService = (item) => {
                        return `checker.option('${fieldName}', ${item}.${fieldName}, Boolean);`;
                    }
        ...
        Date(...yyyy) {
            ...
            Date.gen = (fieldName) => {
                ...
                if(defaultValue.indexOf('auto') === -1) Date.assignUpController = Date.assignInController = `${fieldName}: Date`;
                if (defaultValue === undefined) {
                    Date.validateInService = (item) => {
                        return `checker.must('${fieldName}', ${item}.${fieldName}, Date);`;
                    }
                    Date.validateUpService = (item) => {
                        return `checker.option('${fieldName}', ${item}.${fieldName}, Date);`;
                    }
                    ...
                    Date.validateUpService = (item) => {
                        if(defaultValue.indexOf('auto') === -1) return `checker.option('${fieldName}', ${item}.${fieldName}, Date, ${defaultValue});`;
                        if(defaultValue.indexOf('auto-update') !== -1) return `${item}.${fieldName} = new Date();`;
                        return null;
                    }
                    Date.validateInService = (item) => {
                        if(defaultValue.indexOf('auto') === -1) return `checker.must('${fieldName}', ${item}.${fieldName}, Date, ${defaultValue});`;
            ...
            Object.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {                    
                    const validate = (item, type) => {
                        let cnt = '';
                        if(type === 1) {
                            if(!schema) cnt = `checker.option('${fieldName}', ${item}.${fieldName}, Object);`;
                            else cnt = `checker.option('${fieldName}', ${item}.${fieldName}, Object, (${fieldName})=>{${checkWhenHas('', fieldName, type)}});`;
                        } else {
                            cnt = `checker.must('${fieldName}', ${item}.${fieldName}, Object);`;
                    ...
                    const validate = (item, type) => {
                        ...
                        if(type === 1) {
                            ...
                            cnt = `checker.must('${fieldName}', ${item}.${fieldName}, Object, ${JSON.ostringify(defaultValue, null, '\t').replace(/\n/g, '\n')});`;
                            if(schema) cnt += checkWhenHas(`${item}.`, fieldName, type);
                        }
                        return cnt;
                    };
        ...
        Array(schema, defaultValue) {
            ...
            Array.gen = (fieldName) => {
                ...
                let checkWhenHas = (item, type) => {
                    ...
                    if(rs.length > 0)
                        return `for(${fieldName} of ${item}.${fieldName}) {
                            ${rs.join('\n')}
                        }`;
                    return '';
                }
                ...
                if (defaultValue === undefined) {
                    const validate = (item, type) => {
                        let cnt = '';
                        if(type === 1) {
                            if(!schema) cnt = `checker.option('${fieldName}', ${item}.${fieldName}, Array);`;
                            else cnt = `checker.option('${fieldName}', ${item}.${fieldName}, Object, (${fieldName})=>{${checkWhenHas(item, type)}});`;
                        } else {
                            cnt = `checker.must('${fieldName}', ${item}.${fieldName}, Array);`;
                    ...
                    const validate = (item, type) => {
                        ...
                        if(type === 1) {
                            if(!schema) cnt = `checker.option('${fieldName}', ${item}.${fieldName}, Array);`;
                            else cnt = `checker.option('${fieldName}', ${item}.${fieldName}, Array, (${fieldName})=>{${checkWhenHas(item, type)}});`;
                        } else {
                            cnt = `checker.must('${fieldName}', ${item}.${fieldName}, Array, ${JSON.ostringify(defaultValue, null, '\t')});`;
                            if(schema) cnt += checkWhenHas(item, type);
                        }
                        return cnt;
                    };
            ...
            File.gen = (fieldName) => {
                    ...
                    if (defaultValue === undefined) {
                        File.validateInService = (item) => {
                            return `checker.must('${fieldName}', ${item}.${fieldName}, ${config.multiples ? 'Array' : 'String'});`;
                        }
                        File.validateUpService = (item) => {
                            return `checker.option('${fieldName}', ${item}.${fieldName}, ${config.multiples ? 'Array' : 'String'});`;
                        }
                        ...
                        File.validateInService = (item) => {
                            return `checker.must('${fieldName}', ${item}.${fieldName}, ${config.multiples ? 'Array' : 'String'}, ${JSON.ostringify(defaultValue)});`;
                        }
                        File.validateUpService = (item) => {
                            return `checker.option('${fieldName}', ${item}.${fieldName}, ${config.multiples ? 'Array' : 'String'});`;
                        }
