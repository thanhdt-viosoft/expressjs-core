...
exports = module.exports = {
    ...
    async insert(tableName, obj) {
        ...
        const collection = db.collection(tableName);        
        try {
            if(!(obj instanceof Array)) return await collection.insertOne(obj);        
            return await collection.insertMany(obj).ops;                
        }finally{
            queue.release(db);
        }      
    },
    ...
    async delete(tableName, where) {
        const db = await queue.get();
        const collection = db.collection(tableName);
        if(!_.isPlainObject(where)) where = { _id: exports.uuid(fil) };        
        try {
