...
const MIME_JSON_ENCRYPT = 'encryption/json';

exports = module.exports = {
  encrypt(encryptKey, app){
    if(!encryptKey) return;
    exports._encryptKey = encryptKey;
    app.use((req, res, next) => {
      req.accepts(MIME_JSON_ENCRYPT);
      res.contentType(MIME_JSON_ENCRYPT);        
      const _send = res.send;
      res.send = (data) => {
        _send.apply(res, [encrypt.encryptText(data, exports._encryptKey)]);
      }
      next();
    });
  },
  ...
  jsonHandler(schema){
    ...
    mid.push((req, res, next) => {
      if(req.headers['content-type'] === MIME_JSON_ENCRYPT) {
        let chunks=[];
        req.accepts(MIME_JSON_ENCRYPT);
        ...
        req.on('end', function() {
          ...
          if(req.headers['content-type'] === MIME_JSON_ENCRYPT) req.body = encrypt.decryptText(req.body, exports._encryptKey);
          next();
        });
      } else {
