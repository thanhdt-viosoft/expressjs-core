...
module.exports = (tbl, fieldsKeyType, fieldsType) => {
    ...
    this.find = (fieldsKeyType, fieldsType) => {
        return `app.get('/${tbl}', utils.jsonHandler(), async (req, res, next) => {                    
                    try {
                        let where = {};
    ...
    this.get = (fieldsKeyType, fieldsType) => {
        return `app.get('/${tbl}/:${fieldsKeyType.fieldName}', utils.jsonHandler(), async (req, res, next) => {
                try {                        
    ...
    this.post = (fieldsKeyType, fieldsType) => {
        ...
        if(bodyFileParser.length > 0){            
            ...
            jsonBodyParser = `utils.jsonHandler(${assignIValue.length > 0 ? ("{" + assignIValue.join(", ") + "}") : ''})`;
        }
        assignIValue = assignIValue.join('\n');
        return `app.post('/${tbl}', ${jsonBodyParser}, async (req, res, next) => {
    ...
    this.put = (fieldsKeyType, fieldsType) => {
        ...
        if(bodyFileParser.length > 0){            
            ...
            jsonBodyParser = `utils.jsonHandler(${assignUValue.length > 0 ? ("{" + assignUValue.join(", ") + "}") : ''})`;
        }
        assignUValue = assignUValue.join('\n\t');
        return `app.put('/${tbl}/:${fieldsKeyType.fieldName}', ${jsonBodyParser}, async (req, res, next) => {
    ...
    this.delete = (fieldsKeyType, fieldsType) => {
        return `app.delete('/${tbl}/:${fieldsKeyType.fieldName}', utils.jsonHandler(), async (req, res, next) => {
                    try{
                        const key = db.uuid(${fieldsKeyType.sign}req.params.${fieldsKeyType.fieldName});
